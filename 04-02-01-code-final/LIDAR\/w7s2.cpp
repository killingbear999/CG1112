...
u_result capture_and_display(RPlidarDriver * drv, const char* fname)
{
    ...
    // C7 change while() first condition from if (IS_OK...)
    ans = drv->grabScanData(nodes, count);
    while (ans = drv->grabScanData(nodes, count), IS_OK(ans) || ans == RESULT_OPERATION_TIMEOUT) {
	...
	signal(SIGINT, ctrlc);
        ...
        for (int pos = 0; pos < (int)count ; ++pos) {
            ...
            // to (X,Y) coordinate
            float endptX = centerPtX+distPixel*sin(rad);
